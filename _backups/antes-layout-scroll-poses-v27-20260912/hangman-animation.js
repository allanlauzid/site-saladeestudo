...
(function () {
  ...
  function ensureTopicRevealStyles() {
    ...
    style.textContent =
      '#hangman-topic-reveal{position:fixed;left:50%;' +
  ...
  /* O texto do topico fica fixo na tela, centralizado horizontalmente no
     mesmo eixo central usado pela palavra "Dica:" no HUD do jogo -- sem
     ancora na seta amarela. Precisa ficar FORA do <nav> porque o <nav>
     mora dentro do #hangman-page-camera, que e deslocado (translateX)
     pra revelar a forca; se o topico ficasse la dentro, ele sairia de
     tela junto com a seta assim que a camera panasse. A posicao vertical
     e medida a partir do <nav> (mesma altura de antes), so a horizontal
     e o "pai" do elemento que mudam. Ele e revelado progressivamente
     enquanto o boneco caminha, usando a mesma janela de tempo (walkStart
  ...
  function ensureTopicReveal() {
    ...
    if (nav) {
      ...
      el.style.top = (navRect.top + navRect.height / 2) + 'px';
    }
    return el;
  }
