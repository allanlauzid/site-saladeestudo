...
(function () {
  ...
  function buildUi() {
    ...
    /* Segunda dica opcional: um botao vermelho "+1" (estilo caneta
       permanente) no canto da pilula. No primeiro clique, mostra a
       segunda dica e o botao vira duas setas em circulo; a partir dai
       cada clique so alterna entre as duas dicas, quantas vezes quiser.
       So aparece quando a palavra sorteada tem uma segunda dica (nem
       toda palavra do banco reserva offline tem, por exemplo). */
    var hint2Value = typeof round.hint2 === 'string' ? round.hint2.trim() : '';
    if (hint2Value) {
      var hint2Toggle = document.createElement('button');
      hint2Toggle.type = 'button';
      hint2Toggle.className = 'hangman-hint2-toggle';
      hint2Toggle.setAttribute('aria-label', 'Mostrar outra dica');
      var plusLabel = document.createElement('span');
      plusLabel.className = 'hangman-hint2-plus';
      plusLabel.textContent = '+1';
      hint2Toggle.appendChild(plusLabel);

      var showingSecondHint = false;
      var hint2Revealed = false;
      hint2Toggle.addEventListener('click', function (event) {
        event.stopPropagation();
        showingSecondHint = !showingSecondHint;
        hintText.textContent = showingSecondHint ? hint2Value : baseHintText;
        if (!hint2Revealed) {
          hint2Revealed = true;
