...
(function () {
  ...
  /* Slots da palavra. A chamada sem argumento mantém os seis campos usados
     nos testes anteriores. Quando uma palavra é informada, a fileira e o
     percurso horizontal crescem de acordo com seus caracteres. */
  ...
  function createRound(input) {
    ...
    if (typeof input === 'string') {
      ...
      console.warn('A palavra da forca deve ser uma string ou um objeto { word, hint }.');
      return null;
    }
    ...
    if (!rawWord) {
      return {
        ...
        normalizedWord: '',
        hint: hint,
        characters: Array(DEFAULT_WORD_LENGTH).fill(''),
        letterCount: DEFAULT_WORD_LENGTH,
        isPlaceholder: true
      };
    }
  ...
  function buildSlots(slotsGroup, round) {
    ...
    var characterWidths = round.characters.map(function (character) {
      return character === ' ' ? SPACE_WIDTH : SLOT_WIDTH;
    });
    var naturalWidth = characterWidths.reduce(function (sum, width) {
      return sum + width;
    }, 0) + SLOT_GAP * Math.max(0, round.characters.length - 1);
    var areaWidth = Math.max(SLOT_AREA_WIDTH, naturalWidth);
    var extraWidth = areaWidth - SLOT_AREA_WIDTH;
    ...
    round.characters.forEach(function (character, characterIndex) {
      ...
      if (!isLetter(character) && character !== '') {
        ...
        cursorX += characterWidth + SLOT_GAP;
        return;
      }
      ...
      hitLine.setAttribute('x2', x1 + SLOT_WIDTH);
    ...
    return {
      areaWidth: areaWidth,
      travelX: TRAVEL_X + extraWidth,
      cameraX: CAMERA_X + extraWidth,
      gallowsShiftX: extraWidth,
      walkCycles: Math.max(WALK_CYCLES, Math.ceil(WALK_CYCLES * (TRAVEL_X + extraWidth) / TRAVEL_X)),
      rowCenters: DISASSEMBLY_ROW_ORDER.map(function (_pieceId, index) {
        var firstCenter = SLOT_START_X + SLOT_WIDTH / 2;
        var lastCenter = SLOT_START_X + areaWidth - SLOT_WIDTH / 2;
