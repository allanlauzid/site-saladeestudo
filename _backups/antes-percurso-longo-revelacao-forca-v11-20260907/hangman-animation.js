...
(function () {
  ...
  /* O personagem avança dentro do SVG enquanto o hero inteiro se desloca
     na direção oposta. Isso reproduz uma câmera de página: texto, mesa e
     cadeira mantêm suas posições relativas e nunca se atravessam. */
  var TRAVEL_X = 55;
  ...
  function getHeroShiftPx(parts) {
    ...
    return TRAVEL_X * (rect.width / viewBox.width);
  }
  ...
  function buildTimeline(parts, frames) {
    ...
    slotNodes.forEach(function (slot) {
      ...
      var passProgress = Math.max(0.12, Math.min(0.92, (slotCenter - 47) / TRAVEL_X));
      timeline.to(slot, {
        strokeDashoffset: 0,
        duration: 0.2,
        ease: 'power1.out'
    ...
    timeline.addLabel('gallows-reveal');
    timeline.to(parts.gallows, {
      autoAlpha: 1,
      duration: 0.38,
      ease: 'power1.out'
    }, 'gallows-reveal');
