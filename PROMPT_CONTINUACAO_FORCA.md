...
## Revisão v15 - avanço rapido por clique, toque ou teclado

Allan aprovou o ultimo frame da v14 e pediu um controle para acelerar a
sequencia ate as seis pecas chegarem a fileira na altura do laco.

Alteracoes aplicadas:

- durante a sequencia, clique, toque ou qualquer tecla muda a timeline para
  velocidade `4x`, sem saltar poses;
- os controles existem apenas enquanto a animacao esta ativa e sao removidos
  na conclusao ou no reset;
- os eventos sao capturados antes de chegarem a links ou ao contador dos dez
  cliques;
- cada nova execucao comeca novamente em velocidade normal (`1x`);
- o primeiro acionamento emite `hangman:fast-forward` com
  `detail.timeScale = 4`;
- `index.html` carrega `hangman-animation.js?v=15`;
- `game-jogodavelha.js` e o listener dos dez cliques nao foram alterados;
- backup: `_backups/antes-avanco-rapido-input-v15-20260907/`.

**Pendente:** atualizar com `Ctrl+F5`, executar `window.startHangmanGame()` e
testar separadamente clique, tecla e toque. Confirmar que a sequencia acelera
sem pular e libera os controles ao chegar a fileira final.

---

## Revisão v16 - orientação paisagem no celular

Allan aprovou o avanço rápido da v15 e pediu que a animação da forca seja
exibida horizontalmente em celulares.

Alterações aplicadas:

- ao iniciar em um celular, o código solicita tela cheia e bloqueio nativo em
  orientação `landscape` quando o navegador oferece essas APIs;
- a timeline só começa depois que a área visível estiver realmente em paisagem;
- se o navegador bloquear o giro automático, aparece uma tela orientando o
  usuário a girar o celular, e a animação permanece pausada;
- ao detectar a orientação horizontal, o aviso some e a timeline começa ou
  continua automaticamente;
- se o aparelho voltar para a orientação vertical durante a sequência, ela é
  pausada até retornar à horizontal;
- a orientação paisagem é preservada no quadro final para manter a composição
  da forca e dos membros corretamente enquadrada;
- ao executar o reset, listeners, aviso, bloqueio de orientação e tela cheia
  criados pela forca são liberados;
- computadores não entram nesse fluxo e continuam iniciando imediatamente;
- `index.html` carrega `hangman-animation.js?v=16`;
- `game-jogodavelha.js` e o listener dos dez cliques não foram alterados;
- backup: `_backups/antes-orientacao-mobile-paisagem-v16-20260907/`.

**Pendente:** testar em um celular real iniciando a forca por um gesto do
usuário. Confirmar o giro automático em Android compatível e o aviso de giro
manual em navegadores que recusam o bloqueio, especialmente no iPhone.

---

## Revisão v17 - ilustração móvel de giro e botão para sair

Allan pediu um aviso visual de dois segundos no início mobile e um X vermelho
grande para abandonar a animação a qualquer momento.

Alterações aplicadas:

- todo início em celular mostra por dois segundos uma ilustração SVG de um
  aparelho girando da orientação vertical para a horizontal;
- durante esses dois segundos a timeline permanece parada, mesmo quando o
  navegador consegue girar a tela imediatamente;
- depois dos dois segundos, a animação começa se o viewport estiver em
  paisagem; em modo retrato, a mesma tela permanece como aviso estático;
- foi criado um botão circular com X vermelho de 64 px no canto superior
  direito, acima da animação e do aviso de orientação;
- o X fica disponível do início até o quadro final;
- ao acionar o X, a timeline é interrompida, o mascote e a página retornam ao
  estado original, e tela cheia/orientação móvel são liberadas;
- o X tem prioridade sobre o avanço rápido e seu acionamento não alcança o
  contador dos dez cliques;
- o aviso respeita `prefers-reduced-motion`, exibindo diretamente o aparelho
  na horizontal sem o movimento de rotação;
- `index.html` carrega `hangman-animation.js?v=17`;
- `game-jogodavelha.js` e seu listener não foram alterados;
- backup: `_backups/antes-aviso-giro-e-saida-v17-20260907/`.

**Pendente:** testar no computador se o X restaura a página durante a caminhada
e no quadro final. Em celular, validar os dois segundos da ilustração, o início
após a orientação horizontal e a saída pelo X durante o aviso.

---

## Revisão v18 - X vermelho sem círculo

Allan pediu que o controle de saída mostrasse somente o X vermelho, sem botão
circular aparente.

Alterações aplicadas:

- removidos fundo branco, borda vermelha circular, arredondamento e sombra;
- preservados o X vermelho grande, a posição no canto superior direito e a
  área de clique/toque de 64 px;
- comportamento de saída, prioridade sobre o avanço rápido e acessibilidade
  permanecem inalterados;
- `index.html` carrega `hangman-animation.js?v=18`;
- `game-jogodavelha.js` não foi alterado;
- backup: `_backups/antes-x-vermelho-sem-circulo-v18-20260907/`.

**Pendente:** atualizar com `Ctrl+F5` e validar visualmente o X durante a
animação, no aviso móvel e no quadro final.

---

## Revisão v19 - animações das seis peças flutuantes

Allan aprovou a proposta e pediu a aplicação de todos os movimentos sugeridos
para o quadro final.

Alterações aplicadas:

- a chegada à fileira ganhou acomodação com ultrapassagem de 1,5 unidade e
  retorno de 450 ms usando `back.out(1.4)`;
- as seis peças flutuam de forma assíncrona entre 1,6 e 2,7 unidades, com
  durações diferentes entre 1,8 e 2,35 segundos e easing `sine.inOut`;
- braços e pernas balançam em sentidos alternados entre 2 e 2,5 graus;
- o tronco balança somente 0,8 grau e a cabeça permanece sem rotação;
- uma onda percorre a fileira a cada 5,5 segundos: cada peça sobe 3 unidades,
  com intervalo de 70 ms, e retorna suavemente;
- clicar ou tocar em um campo faz a peça correspondente descer 4 unidades por
  180 ms e retornar em 320 ms;
- os campos receberam áreas transparentes de toque de 12 unidades sem mudar a
  espessura visual dos traços;
- o futuro gameplay pode acionar a mesma reação com
  `HangmanAnimation.reactPiece(indiceOuId)` ou emitindo `hangman:piece-react`
  com `detail.index` ou `detail.pieceId`;
- flutuação, onda e reação usam estados separados e são somadas na renderização,
  evitando tweens concorrentes no mesmo `transform`;
- no modo `prefers-reduced-motion`, a flutuação, onda, reação e acomodação são
  reduzidas e as rotações são removidas;
- o X/reset encerra todos os loops, remove os listeners e restaura rotação e
  posição originais;
- `index.html` carrega `hangman-animation.js?v=19`;
- `game-jogodavelha.js` não foi alterado;
- backup: `_backups/antes-flutuacao-pecas-v19-20260907/`.

**Pendente:** atualizar com `Ctrl+F5`, executar `window.startHangmanGame()` e
validar a acomodação, a flutuação individual, os balanços, a onda e a reação ao
clicar nos seis campos. Confirmar também que o X remove todos os movimentos.

---

## Revisão v20 - ciclos flutuantes duas vezes mais lentos

Allan considerou rápida a flutuação contínua das seis peças e pediu que cada
ciclo tivesse o dobro do tempo.

Alterações aplicadas:

- cabeça: de 1,80 para 3,60 segundos;
- tronco: de 2,05 para 4,10 segundos;
- braço esquerdo: de 2,25 para 4,50 segundos;
- braço direito: de 1,95 para 3,90 segundos;
- perna esquerda: de 2,35 para 4,70 segundos;
- perna direita: de 2,10 para 4,20 segundos;
- distâncias, rotações, easing, onda coletiva, reação ao toque e coreografia
  principal foram preservados;
- `index.html` carrega `hangman-animation.js?v=20`;
- `game-jogodavelha.js` não foi alterado;
- backup: `_backups/antes-ciclos-flutuantes-lentos-v20-20260907/`.

**Pendente:** atualizar com `Ctrl+F5`, executar `window.startHangmanGame()` e
validar se a flutuação final ficou suficientemente lenta e natural.

---

## Revisão v21 - palavra e quantidade de campos dinâmicas

O início da forca passou a aceitar os dados de uma rodada externa, deixando o
código preparado para receber futuramente a palavra sorteada no banco de dados.

Formas de iniciar:

```js
window.startHangmanGame();
window.startHangmanGame('MATEMÁTICA');
window.startHangmanGame({ word: 'ENSINO MÉDIO', hint: 'Etapa escolar' });
```

Alterações aplicadas:

- a chamada sem parâmetros mantém os seis campos de teste anteriores;
- a palavra pode ser enviada diretamente ou no objeto `{ word, hint }`;
- os campos são criados conforme a quantidade real de letras;
- espaços não geram campos e sinais como hífen ficam visíveis sem depender de
  tentativa do jogador;
- acentos são preservados na palavra original e também é gerada uma versão
  normalizada, sem acentos e em maiúsculas, para o futuro gameplay;
- cada campo guarda internamente sua letra, letra normalizada, índice do campo
  e índice do caractere;
- palavras maiores ampliam a área dos campos, a distância da caminhada, o
  acompanhamento da câmera, a quantidade de ciclos de marcha e a posição da
  forca;
- a fileira final das seis peças deixou de depender da existência de exatamente
  seis campos e se distribui por toda a largura disponível;
- o evento `hangman:round-ready` informa palavra, palavra normalizada, dica,
  número de letras e se a rodada usa placeholders;
- os dados atuais podem ser consultados com `HangmanAnimation.getRound()`;
- entradas inválidas e palavras com mais de 40 caracteres são recusadas sem
  interromper uma animação já ativa;
- `index.html` carrega `hangman-animation.js?v=21`;
- `game-jogodavelha.js` e o listener dos dez cliques não foram alterados;
- backup: `_backups/antes-palavras-dinamicas-v21-20260907/`.

**Pendente:** atualizar com `Ctrl+F5` e validar visualmente chamadas com palavra
curta, palavra de seis letras, palavra longa, palavra acentuada e expressão com
espaço ou hífen. O sorteio no banco, teclado, revelação das letras e regras de
acerto/erro continuam pertencendo ao checkpoint de gameplay.

---

## Revisão v22 - largura padrão e campos compactos

Allan considerou excessiva a expansão horizontal da versão v21 e aprovou que a
palavra de seis letras seja a referência de tamanho para todas as rodadas.

Alterações aplicadas:

- seis letras mantêm exatamente a largura total de 160 unidades, com traços de
  20 unidades e espaços de 8;
- entre 7 e 15 letras, a largura dos traços e dos intervalos diminui
  progressivamente para manter a fileira dentro das mesmas 160 unidades;
- referências sem separadores resultam aproximadamente em: 8 letras com traço
  14,75 e espaço 6; 10 letras com traço 11,68 e espaço 4,8; 12 letras com traço
  9,67 e espaço 4; 15 letras com traço 7,68 e espaço 3,2;
- expressões com espaços ou hífens também comprimem os intervalos quando
  necessário, com largura mínima de 5 para cada traço;
- o limite passou a ser 15 letras jogáveis por rodada;
- caminhada, câmera, quantidade de ciclos e posição da forca voltaram aos
  valores padrão, sem aumentar conforme o comprimento da palavra;
- a distribuição final dos seis membros permanece na largura padrão aprovada;
- `index.html` carrega `hangman-animation.js?v=22`;
- `game-jogodavelha.js` e o listener dos dez cliques não foram alterados;
- backup: `_backups/antes-campos-compactos-v22-20260907/`.

**Pendente:** atualizar com `Ctrl+F5` e comparar visualmente palavras de 6, 10
e 15 letras, além de uma expressão curta com espaço ou hífen.

---

## Revisão v23 - dica, teclado animado e gameplay visual

Foi implementado o primeiro gameplay completo da forca após a chegada dos seis
membros à fileira flutuante.

Fluxo implementado:

- `startHangmanGame()` agora usa por padrão `ESTUDO`, com a dica “Atividade
  central da Sala de Estudo”;
- continua possível informar `{ word, hint }` externamente;
- quando a sequência inicial termina, a dica e um teclado QWERTY compacto
  aparecem na parte inferior;
- tocar em qualquer parte do teclado pequeno abre o teclado grande sobre toda a
  tela, com fundo borrado;
- a primeira interação apenas abre o teclado; a letra só é escolhida no teclado
  ampliado;
- a tecla escolhida cresce rapidamente antes de responder;
- acertos recebem um check verde desenhado por `stroke-dashoffset`, persistente
  nos teclados pequeno e grande;
- a letra correta voa do teclado ao primeiro campo correspondente da esquerda;
- repetições nascem do campo anterior e seguem para as próximas ocorrências da
  esquerda para a direita;
- cada pouso revela a letra e produz uma explosão de purpurina colorida;
- em cada acerto, todas as peças ainda livres saltam e giram em comemoração;
- erros recebem um X vermelho desenhado, persistente nos dois teclados;
- a letra errada com o X sai flutuando pela lateral esquerda;
- as peças livres tremem de medo e uma peça segue para a forca;
- ordem fixa dos erros: cabeça, tronco, braço esquerdo, braço direito, perna
  esquerda e perna direita;
- as letras já usadas ficam desativadas nos dois teclados;
- a sexta falha mostra a palavra correta; completar os campos mostra a mensagem
  de vitória;
- cliques e toques dentro da interface não chegam ao listener secreto dos dez
  cliques;
- a dica externa é inserida como texto, sem aceitar HTML;
- movimentos são reduzidos quando `prefers-reduced-motion` está ativo;
- o X/reset remove o teclado e encerra efeitos e animações temporárias;
- foi criado `hangman-gameplay.js?v=1`;
- `index.html` carrega `hangman-animation.js?v=23`;
- `game-jogodavelha.js` e seu listener não foram alterados;
- backup: `_backups/antes-gameplay-teclado-v23-20260907/`.

**Pendente:** validar visualmente o tamanho/posição da dica e dos dois teclados,
o trajeto de letras simples e repetidas, purpurina, comemoração, medo e encaixe
das seis peças na forca. Os alvos das peças podem precisar de ajuste fino após
ver o resultado no navegador.

---

## Revisão v24 - cinco poses engraçadas na forca

Foram incorporadas cinco configurações de pose para a etapa de erros. Cada nova
rodada escolhe uma pose aleatoriamente; também é possível fixá-la externamente
com `window.startHangmanGame({ word, hint, gallowsPose: 0 })`, usando índices de
0 a 4.

Ordens de colocação dos membros:

1. braço esquerdo, tronco, perna esquerda, cabeça, braço direito, perna direita;
2. perna direita, tronco, perna esquerda, cabeça, braço direito, braço esquerdo;
3. cabeça, tronco, braço direito, braço esquerdo, perna esquerda, perna direita;
4. cabeça, tronco, braço direito, perna direita, braço esquerdo, perna esquerda;
5. cabeça, tronco, perna esquerda, perna direita, braço direito, braço esquerdo.

As poses 1 e 2 têm um membro preso ao laço e iniciam um balanço contínuo
compartilhado entre a forca e as peças já penduradas. A madeira agora usa
marrom (`#8B4513`) e o laço usa um tom marrom-bege (`#B88A5A`). O listener dos
dez cliques, o jogo da velha e o restante do site não foram alterados.

**Pendente:** testar uma rodada em cada `gallowsPose`, conferir se os membros
encaixam visualmente no laço/poste e ajustar apenas as coordenadas de cada pose
se necessário.

---

## Revisão v25 - encaixe, fileira e estabilidade da forca

- a desmontagem agora entra mais rapidamente na posição final dos seis membros;
- a fileira final calcula a largura real de cada peça e mantém folga entre elas;
- membros finais recebem interação de mouse/toque e sobem brevemente ao passar;
- a forca permanece fixa, sem a rotação contínua que havia sido aplicada na
  revisão anterior;
- as poses 1 e 2 receberam rotações mais abertas para aproximar o balanço em L
  e a estrela das referências visuais aprovadas;
- `index.html` carrega `hangman-animation.js?v=25`.

**Pendente:** validar no navegador cada uma das cinco poses, especialmente os
pontos de contato com o laço e o poste.

---

## Revisão v26 - transição final e interface elevada

- o tempo de deslocamento dos membros até a fileira voltou ao ritmo normal;
- a transição começa imediatamente após a desmontagem, sem reduzir a duração
  do percurso de cada membro;
- o teclado compacto foi elevado para ficar entre os campos e os membros;
- a dica e a legenda inferior foram ampliadas em aproximadamente três vezes,
  com ajuste específico para telas estreitas;
- `index.html` carrega `hangman-gameplay.js?v=2`.

**Pendente:** atualizar o navegador com `Ctrl+F5` e validar o enquadramento da
dica, do teclado e da legenda em desktop e mobile.

---

## Revisão v27 - encerramento e ancoragem das poses

- a desmontagem mantém o percurso normal dos membros, mas inicia a flutuação
  imediatamente ao concluir a pose final;
- vitória mostra “PARABÉNS! VOCÊ ACERTOU!” e mantém as peças restantes pulando;
- as letras reveladas ficam verdes e pulam durante a comemoração;
- após 15 segundos ou interação do usuário, um apagador grande remove a cena e
  chama o reset da animação;
- derrota mostra a mensagem motivacional solicitada e insere as letras faltantes
  em vermelho com opacidade 0,7;
- “Dica:” fica fora da cápsula amarela; o texto da dica permanece dentro;
- a legenda do teclado desaparece suavemente ao abrir o teclado;
- os alvos das poses passam a usar pontos de ancoragem explícitos, incluindo
  mão/pé no laço e pivôs anatômicos para ombros e quadris;
- `index.html` carrega `hangman-gameplay.js?v=3`.

## Revisão v28 - vitória, derrota e ancoragem anatômica

- vitória agora usa a mensagem exata “PARABÉNS! VOCÊ ACERTOU!”;
- letras reveladas ficam verdes e pulam continuamente até o encerramento;
- os membros restantes também permanecem em comemoração até o reset;
- a rodada termina por clique ou após 15 segundos com um apagador ampliado;
- derrota usa a mensagem motivacional solicitada e mostra letras faltantes em
  vermelho com opacidade 0,7;
- a legenda inferior desaparece em fade ao abrir o teclado;
- “Dica:” foi separado da cápsula amarela;
- as poses na forca usam pivôs e âncoras anatômicas explícitas para evitar
  membros desconectados e prender mão/pé no laço quando aplicável;
- `index.html` carrega `hangman-gameplay.js?v=3`.

**Pendente:** validação visual completa das cinco poses e dos fluxos de vitória
e derrota no navegador.

---

## Checkpoint de interface v29

- “Dica:” passou para uma linha acima da pílula amarela; os dois elementos ficam centralizados em relação ao teclado;
- a instrução “Toque no teclado para escolher uma letra” aparece somente no início, desaparece com fade após a primeira letra escolhida e não retorna durante a rodada;
- mensagens intermediárias de acerto, erro e continuação foram removidas da área abaixo do teclado;
- as mensagens finais de vitória e derrota continuam aparecendo conforme aprovado;
- as cinco poses da forca não foram alteradas neste checkpoint: os encaixes aguardam marcação manual dos pontos anatômicos no Inkscape para evitar novas tentativas imprecisas;
- `index.html` carrega `hangman-gameplay.js?v=4`.

**Pendente:** validar no navegador as cinco poses, a vitória, a derrota e o
apagamento da cena em desktop e mobile.

---

## Checkpoint de posicionamento v30

- o conjunto completo da interface — “Dica:”, pílula, teclado compacto e legenda/mensagem — foi deslocado exatamente 100 px para cima;
- o deslocamento foi aplicado tanto no desktop quanto em telas com até 700 px;
- espaçamentos internos, tamanhos e comportamento das mensagens foram preservados;
- as poses e âncoras do mascote não foram alteradas;
- `index.html` carrega `hangman-gameplay.js?v=5`.

**Pendente:** atualizar o navegador e validar visualmente a nova altura antes de qualquer outro ajuste manual.

---

## Checkpoint de tópico v31

- o formato nativo de `hangman-words.json` passou a ser aceito diretamente por `startHangmanGame`, com os campos `topico`, `palavra` e `charada`;
- os nomes alternativos `topic`, `word` e `hint` continuam compatíveis;
- a interface mostra “Tópico: [nome]” acima de “Dica:”, centralizado no mesmo eixo da pílula e do teclado;
- quando não for fornecido tópico, a interface usa “Tópico: Geral”;
- o sorteio e o acionamento externo não foram alterados neste checkpoint;
- `index.html` carrega `hangman-animation.js?v=26` e `hangman-gameplay.js?v=6`.

Exemplo com uma entrada do JSON:
...
**Pendente:** escolher o método de definição manual das cinco poses finais e registrar pontos de ancoragem exatos no Inkscape.

---

## Checkpoint de montagem manual v32

- foi criada a prancha editável `outputs/mascote-montagem-5-poses-forca.svg` no diretório de trabalho do Codex;
- a prancha contém as seis formas azuis reais usadas depois da desmontagem: cabeça, tronco, dois braços e duas pernas;
- braços preservam cotovelos como subarticulações e pernas preservam joelhos como subarticulações;
- as cinco poses aprovadas aparecem como referências semitransparentes, acompanhadas pela forca e pelo laço;
- a camada `MONTAGEM — COLE AS PEÇAS AQUI` está livre para Allan duplicar e posicionar as peças;
- instruções foram salvas em `outputs/COMO_MONTAR_5_POSES_NO_INKSCAPE.md`;
- o site e as poses em produção não foram alterados neste checkpoint.

**Pendente:** Allan montar as cinco poses, salvar o SVG resultante e informar o caminho do arquivo para extração das transformações finais.
## Checkpoint v28 — teclado nítido e título revelado pela câmera

- o desfoque do teclado ampliado foi removido; somente o cenário fora da interface recebe desfoque;
- o título do tópico permanece inteiro desde o início e entra no enquadramento apenas pelo deslocamento da câmera enquanto o mascote caminha;
- a posição do título continua pertencendo ao cenário, e não à tela do usuário;
- a causa pendente do boneco torto foi registrada: a montagem precisa usar os pivôs anatômicos do SVG (mão, ombro, quadril, joelho e pé), não apenas centros geométricos ao pendurar cada peça;
